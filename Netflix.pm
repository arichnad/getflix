...
sub getRatings {
	...
	my $cur = 1;
	my $genre ='';
	while ( $body !~ /next-inactive|You have not rated any movies|<title>Netflix - Member Login/i ) {
...
# this regular expression will need to change as well.
		while ( $body =~ s/^.*?(?:\sid="b([^"]+)_0"|unav).*?>\s*([^<]+?)\s*<.*?(?:you rated this movie: (\d)|\b(sbmfpr)\b)//gsi ) {
			next if $4 =~ /sbmfpr/;
			print FD "$1~$2~~~~$3\n";
		}
		++$cur;
